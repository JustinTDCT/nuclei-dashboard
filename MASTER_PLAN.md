...
- **Scale S3 — Central maintenance/query paths.** Agent challenge nonces live in PostgreSQL. Split:
  ...
  - **S3D — True API pagination.** Current. Inventory collection GETs first; change high-risk inventory/scan-volume lists before config lists. High-risk staff lists return the existing `HistoryPage` envelope (`items`, `total`, `limit`, `offset`) with default 50 and max 200. Filters still apply before the limit. This tranche does not rewrite every router, does not keyset report iteration, and does not add a second API replica. Schema stays `0017_security_h6_h8`; `0018` is not created. Agent pin stays `3cdb52c`.
  - Later S3: keyset report iteration. A deliberate multi-API replica gate only after those central work/query paths are bounded.
...
- **Sec H9 — Horizontal API scale remains later S3 work.** Challenge nonces live in PostgreSQL. APScheduler ownership moved to the dedicated scheduler process in S3B, so API replicas no longer start duplicate schedulers. S3D does not add a second API replica.
