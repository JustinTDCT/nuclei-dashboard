...
- **Scale S3 — Central maintenance/query paths.** Agent challenge nonces now live in PostgreSQL; in-process APScheduler still forbids API replicas until S3B. Split:
  - **S3A — Remove startup whole-Device refresh.** Current. Eliminate `refresh_discovery_metadata` from API process startup without changing Device classification / `auto_label` / tech semantics. Live ingest already applies those heuristics per observation. Residual catch-up is one keyset-bounded Device page per scheduler tick (`DISCOVERY_METADATA_BATCH_SIZE`, default 250), not `query(Device).all()`. API startup Device SQL must be independent of inventory size. Schema stays `0017_security_h6_h8`; `0018` is not created. Do not extract APScheduler from the API process in this tranche.
