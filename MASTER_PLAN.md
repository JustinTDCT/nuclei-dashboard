...
- **Scale S3 — Central maintenance/query paths.** Agent challenge nonces live in PostgreSQL. Split:
  ...
  - **S3B — Scheduler process separation.** Current. Remove APScheduler ownership from the API process so two or more API replicas cannot create duplicate scheduler ownership. API startup/shutdown have zero scheduler side effects. Existing job ids and intervals stay unchanged. Exactly one independently deployable scheduler process holds a PostgreSQL session-level leader lock (`pg_try_advisory_lock`) and keeps proving that connection is the same backend (`pg_backend_pid`) before it is allowed to run jobs. Session loss stops APScheduler with `wait=True` and exits; graceful shutdown also waits for in-flight jobs before unlocking. Schema stays `0017_security_h6_h8`; `0018` is not created. This tranche does not add a second API replica.
...
- **Sec H9 — Horizontal API scale remains later S3 work.** Challenge nonces live in PostgreSQL. APScheduler ownership moved to the dedicated scheduler process in S3B, so API replicas no longer start duplicate schedulers. This tranche does not add a second API replica.
