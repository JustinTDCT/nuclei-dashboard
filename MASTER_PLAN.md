...
- **Scale S3 — Central maintenance/query paths.** Agent challenge nonces live in PostgreSQL. Split:
  ...
  - **S3E — Keyset report/export iteration.** Current. Exports keep full-result CSV column/order/filter semantics while never materializing the full result set in Python memory. Iteration is deterministic keyset batches, not `OFFSET` and not `.all()`. Staff `HistoryPage` preview/list contracts stay S3D. This tranche does not add a second API replica. Schema stays `0017_security_h6_h8`; `0018` is not created. Agent pin stays `3cdb52c`.
  - Later S3: a deliberate multi-API replica gate only after those central work/query paths are bounded.
...
- **Sec H9 — Horizontal API scale remains later S3 work.** Challenge nonces live in PostgreSQL. APScheduler ownership moved to the dedicated scheduler process in S3B, so API replicas no longer start duplicate schedulers. S3D and S3E do not add a second API replica.
