...
- **Scale S3 — Central maintenance/query paths.** Agent challenge nonces live in PostgreSQL. Split:
  ...
  - **S3F — Replica-readiness inventory and two-API gate.** Current. Inventory remaining process-local state and filesystem assumptions, then prove two API processes against the same PostgreSQL and shared `scan-artifacts` volume. The dedicated scheduler stays single-active; do not `--scale scheduler=2`. Auth, staff requests, Agent/scanner traffic, writes, and artifact reads must succeed through both replicas. Default Compose still runs one API; this tranche does not declare horizontal scaling complete. Schema stays `0017_security_h6_h8`; `0018` is not created. Agent pin stays `3cdb52c`.
...
- **Sec H9 — Horizontal API scale is not complete.** Challenge nonces and login lockouts live in PostgreSQL. APScheduler ownership moved to the dedicated scheduler process in S3B, so API replicas no longer start duplicate schedulers. S3A–S3E did not add a second API replica. S3F inventories remaining process-local/filesystem assumptions and adds a two-API gate; it does not make two API replicas the live operating model and does not scale the scheduler.
