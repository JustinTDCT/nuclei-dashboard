...
- **Scale S3 — Central maintenance/query paths.** Complete / frozen. Agent challenge nonces live in PostgreSQL. Split:
  ...
  - **S3C — EventAlertQueue stale reclaim.** Accepted at `07d883c`. A queue row left `processing` after a crash is reclaimable after `ALERT_QUEUE_LEASE_SECONDS` using existing `updated_at` (set at claim). A recently claimed row is not stolen. Max-attempt rules still apply. Claim stays `FOR UPDATE SKIP LOCKED` and `ALERT_ROUTE_BATCH_SIZE`. Reclaim runs inside the existing `alert-routing` tick; the 12-job catalog is unchanged. Schema stayed at `0017_security_h6_h8`; `0018` was not created. Agent pin stayed `3cdb52c`. This tranche did not add a second API replica. The pending index is `(status, next_attempt_at)`; stale reclaim filters `(status='processing', updated_at <= cutoff)` and can use the status prefix. Do not add `0018` for a dedicated `(status, updated_at)` index unless a large-queue plan shows a real problem.
  - **S3D — True API pagination.** Accepted at implementation `782f072` plus UI correction `fcc47e5`. Inventory collection GETs first; high-risk staff lists return the existing `HistoryPage` envelope (`items`, `total`, `limit`, `offset`) with default 50 and max 200. Filters still apply before the limit. Agent/scanner poll contracts stay lists. Assets search Enter resets to offset 0. This tranche did not rewrite every router, did not keyset report iteration, and did not add a second API replica. Schema stayed `0017_security_h6_h8`; `0018` was not created. Agent pin stayed `3cdb52c`.
  - **S3E — Keyset report/export iteration.** Accepted at implementation `b241ee9` plus correction `a6eabbe`. Report CSV/PDF table rows and compatibility Assets/Devices/Findings CSV iterate deterministic keyset batches (not `OFFSET`, not `.all()`), expunge ORM rows between batches, and keep full-result export semantics. Executive open-age bands are six PostgreSQL `CASE` + `GROUP BY` rows, not an O(N) `first_seen` load. Resolved-finding export keysets on `COALESCE(resolved_at, latest resolution-history timestamp)` as selected `sort_at`, not `first_seen`. Preview and staff `HistoryPage` lists stay S3D offset pagination. Schema stayed `0017_security_h6_h8`; `0018` was not created. Agent pin stayed `3cdb52c`. This tranche did not add a second API replica.
  - **S3F — Replica-readiness inventory and two-API gate.** Accepted / frozen. Implementation `ab9fa42`, Caddy failover `a856af8`, dynamic-upstream health-check correction `fd697a6`. Docs checkpoint `4735e57` remains the intermediate pre-live record. Live Compose `--scale api=2` passed: two healthy APIs against shared PostgreSQL and `scan-artifacts`, exactly one scheduler leader, Caddy `dynamic a api 8000`, replica stop with 200/200 GETs and zero 502/503/timeouts including requests overlapping `docker stop`, staff auth/read/write on the surviving replica, LAN Agent traffic, new LAN job 10 `done`, central WAN scanner traffic, new WAN job 11 `done`, new raw artifacts from both paths readable through Caddy. Discovery-profile runs with no Nuclei stage and zero findings keep existing meaning. Caddy protection is 1s DNS refresh, 500ms `transport http` `dial_timeout` (not `dynamic a` `dial_timeout`), 2s `lb_try_duration`, and 2s passive `fail_duration`; do not use `health_uri` on dynamic upstreams; do not replay POST/PATCH after a connected round-trip. One or two API replicas are supported; Compose default remains one API. Do not `--scale scheduler=2`. Schema stayed `0017_security_h6_h8`; `0018` was not created. Agent pin stayed `3cdb52c`.
...
- **V1B — Operational Release Readiness.** ACCEPT / CLOSED. Implementation/ops baseline: `bb63c6b` (PR #1). Acceptance checkpoint: `39f463c0` (merge of PR #2). Evidence: `docs/V1B_CLOSURE.md`. Runbook: `docs/V1B_OPERATIONS.md`.
- **V1C — Technician/auditor UX walk.** ACCEPT / CLOSED. Walked live HEAD `39f463c0`. No V1C blockers. Known V1A PARTIALs remain backlog. Evidence: `docs/V1C_CLOSURE.md`. Acceptance checkpoint: merge of the V1C docs PR.
- **V1D — Operational soak.** READY TO START. Final technical gate before a V1 Release Decision. Runbook: `docs/V1D_OPERATIONS.md`. After V1D: V1 release decision → only then a V1 tag and a V1.1/V2 roadmap.
