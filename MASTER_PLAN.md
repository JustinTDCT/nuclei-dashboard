...
- **Scale S3 — Central maintenance/query paths.** Agent challenge nonces live in PostgreSQL. Split:
  ...
  - **S3F — Replica-readiness inventory and two-API gate.** Current. Code / pre-live gate accepted at `ab9fa42` (exactly one commit on frozen S3E `a6eabbe`). Not ACCEPT / FROZEN. Local pytest starts two uvicorn processes against shared PostgreSQL and `RAW_ARTIFACT_DIR`; that does not prove Compose named-volume sharing or Caddy dynamic upstreams. The live gate is `--scale api=2` with one scheduler, then health/staff/Agent/scanner/artifact traffic through Caddy plus a single-API recreate. H9 stays open until that live gate is accepted. Default Compose remains one API. Schema stays `0017_security_h6_h8`; `0018` is not created. Agent pin stays `3cdb52c`.
...
- **Sec H9 — Horizontal API scale is not complete.** Challenge nonces and login lockouts live in PostgreSQL. APScheduler ownership moved to the dedicated scheduler process in S3B, so API replicas no longer start duplicate schedulers. S3A–S3E did not add a second API replica. S3F `ab9fa42` is code / pre-live gate accepted, not frozen. Two API replicas are not the live operating model until the Compose `--scale api=2` gate (Caddy dynamic upstreams, shared `scan-artifacts`, single scheduler) is accepted. Do not `--scale scheduler=2`.
