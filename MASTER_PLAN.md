...
- **Scale S3 — Central maintenance/query paths.** Agent challenge nonces live in PostgreSQL. Split:
  ...
  - **S3F — Replica-readiness inventory and two-API gate.** Current. Code / pre-live gate accepted at `ab9fa42` (docs checkpoint `4735e57`). Not ACCEPT / FROZEN. Live Compose `--scale api=2` is a **PARTIAL PASS**: two healthy APIs, shared `scan-artifacts`, one scheduler leader, schema `0017`, pin `3cdb52c`, Caddy dynamic discovery, public `/api/internal*` 404, staff JWT and DB writes across replicas, existing artifact download, LAN Agent heartbeats. Single-replica removal is PARTIAL because Caddy returned one 502 on an ordinary GET while DNS still held the dead address. New LAN/WAN runs were not executed. Remaining: Caddy fail-over for a dead replica (1s DNS refresh, 500ms `transport http` `dial_timeout`, 2s `lb_try_duration`, 2s passive `fail_duration`; not active `health_uri`, which does not run for `dynamic a` upstreams; not backend or PostgreSQL; do not replay POST/PATCH after a connected round-trip), then 100 `/api/health` plus 100 authenticated GETs with zero 502s while one API stays down for several seconds, then one small authorized LAN scan and one small authorized WAN scan with artifact download through Caddy. H9 stays open; two API replicas are not a supported operating model while that 502 is reproducible. Default Compose remains one API. Do not `--scale scheduler=2`. Schema stays `0017_security_h6_h8`; `0018` is not created. Agent pin stays `3cdb52c`.
...
- **Sec H9 — Horizontal API scale is not complete.** Challenge nonces and login lockouts live in PostgreSQL. APScheduler ownership moved to the dedicated scheduler process in S3B, so API replicas no longer start duplicate schedulers. S3A–S3E did not add a second API replica. S3F `ab9fa42` is code / pre-live gate accepted, not frozen. Live two-API Compose is a partial pass; a planned replica recycle still produced a user-visible 502 on a GET. Two API replicas are not a supported operating model until Caddy fails over dead upstreams for read/health traffic with zero 502s on that recycle, and new LAN/WAN runs prove shared-volume ingest. This is not a claim of zero-overlap HA under every network partition. Do not `--scale scheduler=2`.
