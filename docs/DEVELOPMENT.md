...
The defined implementation roadmap is complete through Phase 3C, Scale S3, and Sec H9 (`3f702b8`). **V1A** is ACCEPT / CLOSED (`a06e455` on audited baseline `3f702b8`). **V1B** is ACCEPT / CLOSED: implementation/ops baseline `bb63c6b` (PR #1); acceptance checkpoint `39f463c0` (PR #2). **V1C** is ACCEPT / CLOSED (`docs/V1C_CLOSURE.md`; walked live HEAD `39f463c0`; no blockers; V1A PARTIALs remain backlog). Current tranche is **V1D — operational soak** (`docs/V1D_OPERATIONS.md`). Do not add `0018`, bump the Agent pin, or fix V1A PARTIALs because V1D exists.
...
GitHub Actions runs backend pytest plus the frontend typecheck/lint/test/build. Job names are `backend` and `frontend`. Frontend CI needs Node **22.10+** because jsdom 30’s undici requires `worker_threads.markAsUncloneable` (absent on Node 20). **V1B `main` protection is active** as repository ruleset `22025478` (“V1B main protection”): PRs required, checks `backend` and `frontend` required and must be up to date, force-push and deletion forbidden, no bypass actors. Direct pushes to `main` are prohibited, including for repository admins. That is a GitHub setting, not application code.
